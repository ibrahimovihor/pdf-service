      ...
      case '[documentItems]':
        ...
        return documentItems.map((item, index) => `
          <tr>
            ...
            <td>${item.articleNumber || ''}</td>
            <td>${item.articleName || ''}</td>
            <td style="text-align: right">${(item.taxRate || 19).toLocaleString('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}%</td>
            <td style="text-align: right">${(item.price || 0).toLocaleString('de-DE', { style: 'currency', currency: 'EUR' })}</td>
            <td style="text-align: right">${(item.total || 0).toLocaleString('de-DE', { style: 'currency', currency: 'EUR' })}</td>
          </tr>
        `).join('')
